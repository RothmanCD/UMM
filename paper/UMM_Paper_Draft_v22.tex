...
\label{eq:A-global}
\end{equation}
A robustness re-run at \(\Upsilon_*=0.7\) shifts the median UMM residual by \(\Delta\epsilon\approx -0.007\) and is not used for the primary numbers.
...
\midrule
SPARC disks & \(\sim\) order unity \\
Classical dSphs & \(1.2\)--\(5.3\) \\
High-residual UDGs & \(\sim0.6\)--\(2.6\) \\
DM-deficient UDGs (DF2, DF4, \ldots) & \(28\)--\(43\) \\
\bottomrule
\end{tabular}
\end{center}
...
A one-paragraph packing sketch of the redshift evolution is nevertheless immediate. Mean cosmic baryon density scales as \(\rho_b\propto(1+z)^3\), so the cosmic separation variable shifts as
\begin{equation}
...
\label{eq:Lambda-z}
\end{equation}
At low redshift the argument of the exponential is large and negative inside the joint-normalization window, so \(\Lambda_{\rm eff}\) is nearly constant; at sufficiently high redshift the packing of independent cycles increases and \(\Lambda_{\rm eff}\) rises toward the high-flux plateau. Until a back-reacted solution exists, statements about \(\Lambda_{\rm eff}(z)\) remain provisional.
...
The principal remaining technical tasks are a fully (or limited-case) back-reacted solution of the higher-dimensional Einstein equations with independent flux, a fully pure first-principles evaluation of the relative unpacking \(\Delta\rho/\rho\) (beyond the packing estimate that currently bounds the conversion running strengths) and of the residual correlation timescale \(\tau_{\rm res}\), a more precise microscopic evaluation of the packing coefficients, and a worked cosmological sector that tracks the redshift evolution of \(\Lambda_{\rm eff}\) beyond the packing sketch~\eqref{eq:Lambda-z}.

\paragraph{SPARC residual table (documented public-catalog analysis).}
A documented public-catalog residual table now exists for \(N=88\) galaxies (selection\_id \texttt{ea6c92761ffd3c5a}; pre-registered cuts on the Lelli, McGaugh \& Schombert 2016 mass models), with measured full-sample medians \(\mathrm{med}(\epsilon_{\rm umm})\approx 0.098\), \(\mathrm{med}(\epsilon_{\rm baryon})\approx 0.507\), \(\mathrm{med}(\epsilon_{\rm nfw})\approx 0.012\), fraction with \(\epsilon_{\rm umm}<0.25\) of \(\approx 0.85\), and fitted single global \(A=(1.41\pm 0.07)\times 10^{-11}\,\mathrm{m\,s^{-2}}\) (\(\Upsilon_*=0.5\), \(n_g=0.4\)). The numbers quoted in Sec.~\ref{sec:SPARC} are those measured products. The residual table, selection manifest, production report, and analysis notebooks are provided as public supplementary material and will be archived under a permanent identifier upon submission. Remaining near-term refinements include THINGS-quality HI map re-reduction for outer-disk gas \cite{Walter2008} and multi-parameter \(\Lambda\)CDM halo comparisons beyond the simple one-parameter NFW baseline.

\paragraph{Domain of applicability (pressure-supported systems).}
The intermediate-regime thin-disk specialization is extended to pressure-supported systems by the residual-geometry-constrained conversion operator of Sec.~\ref{sec:udg-domain}, which is explicitly marked as a domain extension. Under a constrained pure-description trial set lying inside the packing-derived range \(\Delta\rho/\rho\sim0.25\)--\(0.65\), residual ratios for classical dSphs and high-residual UDGs are brought near order unity while SPARC disks remain protected by \(\mathcal{I}\approx0\). Extreme DM-deficient objects (DF2, DF4) are treated as candidates under the residual-memory hypothesis; the large multiplicative corrections required for those systems are illustrative rather than a completed derivation. The logistic amplitude \(A\) is unchanged. The pure first-principles evaluation of \(\Delta\rho/\rho\) (beyond the packing estimate) and of \(\tau_{\rm res}\) remains future work; the claim of no additional free functions beyond the joint-normalization window is restricted to the core thin-disk specialization and residual-charge sector. A complete residual-charge spectrum under mild running of \(D_{\rm eff}\) is likewise open and is not a retuning target for \(A\).
